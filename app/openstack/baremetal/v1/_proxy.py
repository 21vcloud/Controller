...
class Proxy(proxy.Proxy):
    ...
    def set_node_console_state(self, node, enabled=False):
        """Start or stop the serial console.

        :param node: The value can be the name or ID of a node or a
            :class:`~openstack.baremetal.v1.node.Node` instance.
        :param enabled:  Indicates whether console access is enabled or disabled on this node.
        ...
        """
        res = self._get_resource(_node.Node, node)
        return res.set_node_console_state(self, enabled=enabled)
