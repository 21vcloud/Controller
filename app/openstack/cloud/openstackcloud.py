...
class _OpenStackCloudMixin(_normalize.Normalizer):
    ...
    @_utils.valid_kwargs('description', 'name')
    ...
    def add_block_mapping(self, server_id, volume_id, device):
        payload = {'volumeId': volume_id,
                   'device': device}
        data = _adapter._json_response(
            self.compute.post(
                '/servers/{server_id}/block_mappings'.format(
                    server_id=server_id),
                json=dict(block_device_mappings=payload)),
            error_message="Error add block mappings {volume_id} to server "
                          "{server_id}".format(volume_id=volume_id,
                                               server_id=server_id))
        return data

    def delete_block_mapping(self, server_id, volume_id):
        _adapter._json_response(self.compute.delete(
            '/servers/{server_id}/block_mappings/{volume_id}'.format(
